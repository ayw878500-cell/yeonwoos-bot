...
(참고: `LEVERAGE`는 1~30 범위에서 바꿔서 사용하세요.)

```env
LEVERAGE=10  # 1~30 설정 가능
ENTRY_FRACTION=1.0
...
4. 기본 모드(`FULL_BALANCE_ENTRY=true`)에서는 Quantity 비중을 100%로 강제합니다. (내부적으로 `ENTRY_FRACTION=1.0` 고정)
   신호 발생 시 실시간 가용잔고(`available`) 전액으로 바로 진입 시도합니다.
   잔고초과(code=40762)가 발생하면 해당 신호는 추가 알림 없이 스킵하고 다음 신호를 대기합니다.
5. `FULL_BALANCE_ENTRY=false`일 때만 실시간 가용잔고(`available`) × `ENTRY_FRACTION` 방식으로 진입합니다.
