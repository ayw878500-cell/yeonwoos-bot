...
def load_settings() -> Settings:
    ...
    if settings.full_balance_entry:
        # FULL_BALANCE_ENTRY=true 는 항상 수량 비중 100% 의미
        settings.use_available_balance_sizing = True
        settings.entry_fraction = 1.0
...
def _validate(s: Settings) -> None:
    if s.leverage < 1 or s.leverage > 30:
        raise ConfigError("LEVERAGE는 1~30 범위여야 합니다.")
